                            ...
                            <tbody>
                                @foreach ($records as $record)
                                    <tr>
                                        ...
                                        <td>
                                            @if (!empty($record->stripe_price_id) && $record->price > 0)
                                                @if (!auth()->user()->hasRole('Super Admin'))
                                                    <form action="{{ route('session') }}" method="POST">
                                                        @csrf
                                                        <input type="hidden" name="package" value="{{ $record->id }}">
                                                        <button class="mx-1 btn btn-success" type="submit"
                                                            id="checkout-live-button">Subscribe Now!</button>
                                                    </form>
                                                @endif
                                            @endif
